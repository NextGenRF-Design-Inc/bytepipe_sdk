...
int32_t Adrv9001_LoadProfile( adrv9001_t *Instance )
{
  ...
  //  if( PhyAdrv9001_CalibrateSsi( phy ) != 0 )
  //    printf("Failed SSI Calibration\r\n");
  ...
  if( Instance->Params->HwVer == 2 )
  {
    ...
    if((status = adi_adrv9001_gpio_ManualAnalogOutput_Configure(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_PIN_NIBBLE_07_04 )) != 0 )
      return status;

    /* Enable TCXO */
    if((status = adi_adrv9001_gpio_OutputPinLevel_Set(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_07, ADI_ADRV9001_GPIO_PIN_LEVEL_HIGH)) != 0)
      return status;

    /* Disable Rx1A */
    if((status = adi_adrv9001_gpio_OutputPinLevel_Set(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_06, ADI_ADRV9001_GPIO_PIN_LEVEL_LOW)) != 0)
      return status;

    /* Disable Rx2A */
    if((status = adi_adrv9001_gpio_OutputPinLevel_Set(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_05, ADI_ADRV9001_GPIO_PIN_LEVEL_LOW)) != 0)
      return status;
  ...
  else if( Instance->Params->HwVer == 3 )
  {
    ...
    if((status = adi_adrv9001_gpio_ManualAnalogOutput_Configure(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_PIN_NIBBLE_03_00 )) != 0 )
      return status;

    if((status = adi_adrv9001_gpio_ManualAnalogOutput_Configure(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_PIN_NIBBLE_07_04 )) != 0 )
      return status;

    if((status = adi_adrv9001_gpio_ManualAnalogOutput_Configure(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_PIN_NIBBLE_11_08 )) != 0 )
      return status;
    ...
    /* Disable Rx1A */
    if((status = adi_adrv9001_gpio_OutputPinLevel_Set(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_01, ADI_ADRV9001_GPIO_PIN_LEVEL_LOW)) != 0)
      return status;

    /* Disable Rx2A */
    if((status = adi_adrv9001_gpio_OutputPinLevel_Set(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_09, ADI_ADRV9001_GPIO_PIN_LEVEL_LOW)) != 0)
      return status;

    /* Disable Tx1 */
    if((status = adi_adrv9001_gpio_OutputPinLevel_Set(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_00, ADI_ADRV9001_GPIO_PIN_LEVEL_LOW)) != 0)
      return status;

    /* Disable Tx2 */
    if((status = adi_adrv9001_gpio_OutputPinLevel_Set(&Instance->Device, ADI_ADRV9001_GPIO_ANALOG_08, ADI_ADRV9001_GPIO_PIN_LEVEL_LOW)) != 0)
      return status;
...
int32_t Adrv9001_ToPrimed( adrv9001_t *Instance, adi_common_Port_e port, adi_common_ChannelNumber_e channel )
{
  ...
  /* Set Enable Pin */
  AxiAdrv9001_SetEnablePin( Instance->CtrlBase, port, channel, false );

  /* Set Radio State */
  if((status = adi_adrv9001_Radio_Channel_ToPrimed( &Instance->Device, port, channel)) != 0 )
    return status;
...
int32_t Adrv9001_ToCalibrated( adrv9001_t *Instance, adi_common_Port_e port, adi_common_ChannelNumber_e channel )
{
  ...
  /* Set Radio State */
  if((status = adi_adrv9001_Radio_Channel_ToCalibrated(&Instance->Device, port, channel)) != 0 )
    return status;
...
int32_t Adrv9001_ToRfEnabled( adrv9001_t *Instance, adi_common_Port_e port, adi_common_ChannelNumber_e channel )
{
  ...
  if( port == ADI_TX )
  {
    uint16_t attn = channel == ADI_CHANNEL_1? Instance->Params->Tx1Attn : Instance->Params->Tx2Attn;

    if((status = adi_adrv9001_Tx_Attenuation_Set(&Instance->Device, channel, attn)) != 0 )
      return status;

    uint8_t boost = channel == ADI_CHANNEL_1? Instance->Params->Tx1Boost : Instance->Params->Tx2Boost;

    if((status = adi_adrv9001_Tx_OutputPowerBoost_Set(&Instance->Device, channel, boost )) != 0 )
    return status;
  }
  ...
  /* Set Enable Pin */
  AxiAdrv9001_SetEnablePin( Instance->CtrlBase, port, channel, true );

  /* Set Radio State */
  if((status = adi_adrv9001_Radio_Channel_ToRfEnabled(&Instance->Device, port, channel)) != 0 )
    return status;
  ...
  for(uint8_t clkDly = 0; clkDly < 8; clkDly++)
  {
    ...
    for(uint8_t dataDly = 0; dataDly < 8; dataDly++)
    {
      ...
      if(port == ADI_TX)
      {
        ...
        AxiAdrv9001_SetDataPath( Instance->CtrlBase, port, channel, 1);

        AxiAdrv9001_SetData( Instance->CtrlBase, port, channel, ADRV9001_TEST_MODE_PATTERN);
      }
      else
      {
      ...
      if( port == ADI_TX )
      {
        ...
        if((status = adi_adrv9001_Ssi_Tx_TestMode_Status_Inspect( &Instance->Device, channel, ADI_ADRV9001_SSI_TYPE_LVDS, ADI_ADRV9001_SSI_FORMAT_16_BIT_I_Q_DATA, &Cfg, &s )) != 0)
          return status;
      ...
      {
        uint32_t Data;
        AxiAdrv9001_GetData( Instance->CtrlBase, port, channel, &Data );
      ...
      AxiAdrv9001_SetDataPath(  Instance->CtrlBase, port, channel, 0);
    }
  }

  adi_adrv9001_RxSsiTestModeCfg_t RxCfg = { .testData = ADI_ADRV9001_SSI_TESTMODE_DATA_NORMAL };
  if((status = adi_adrv9001_Ssi_Rx_TestMode_Configure(&Instance->Device, channel, ADI_ADRV9001_SSI_TYPE_LVDS, ADI_ADRV9001_SSI_FORMAT_16_BIT_I_Q_DATA, &RxCfg )) != 0)
    return status;

  adi_adrv9001_TxSsiTestModeCfg_t TxCfg = { .testData = ADI_ADRV9001_SSI_TESTMODE_DATA_NORMAL };
  if((status = adi_adrv9001_Ssi_Tx_TestMode_Configure(&Instance->Device, channel, ADI_ADRV9001_SSI_TYPE_LVDS, ADI_ADRV9001_SSI_FORMAT_16_BIT_I_Q_DATA, &TxCfg )) != 0)
    return status;
...
int32_t Adrv9001_SetPaEnable( adrv9001_t *Instance, adi_common_Port_e port, adi_common_ChannelNumber_e channel, bool Enable )
{
  ...
  return adi_adrv9001_gpio_OutputPinLevel_Set(&Instance->Device, pin, level);
}
...
int32_t Adrv9001_SetSsiDataDelay( adrv9001_t *Instance, adi_common_Port_e port, adi_common_ChannelNumber_e channel, uint8_t Delay )
{
  ...
  if((status = adi_adrv9001_Radio_Channel_State_Get( &Instance->Device, port, channel, &State )) == 0)
  {
    if( State != ADI_ADRV9001_CHANNEL_RF_ENABLED )
    {
      ...
      if((status = adi_adrv9001_Ssi_Delay_Inspect(&Instance->Device, ADI_ADRV9001_SSI_TYPE_LVDS, &cal)) == 0)
      {
        if((port == ADI_RX) && (channel == ADI_CHANNEL_1))
        {
        ...
        status = adi_adrv9001_Ssi_Delay_Configure(&Instance->Device, ADI_ADRV9001_SSI_TYPE_LVDS, &cal);
      }
    }
    else
    {
      status = Adrv9001Status_Busy;
    }
  }
...
int32_t Adrv9001_EnableDac( adrv9001_t *Instance, uint8_t Id, bool Enable )
{
  ...
  switch( Id )
  {
    ...
    default: return Adrv9001Status_InvalidParameter;
  }

  if( adi_adrv9001_gpio_Configure(&Instance->Device, signal, &Cfg) != 0)
    return Adrv9001Status_GpioError;
...
int32_t Adrv9001_SetVcTcxo( adrv9001_t *Instance, float Voltage )
{
  ...
  if( Instance->Params->HwVer == 2 )
    return adi_adrv9001_AuxDac_Code_Set(&Instance->Device, ADI_ADRV9001_AUXDAC0, Value);
  else if( Instance->Params->HwVer == 3 )
    return adi_adrv9001_AuxDac_Code_Set(&Instance->Device, ADI_ADRV9001_AUXDAC3, Value);
  else
    return Adrv9001Status_InvalidParameter;
}
...
int32_t Adrv9001_GetVcTcxo( adrv9001_t *Instance, float *Voltage )
{
  ...
  if( Instance->Params->HwVer == 2 )
    status = adi_adrv9001_AuxDac_Code_Get(&Instance->Device, ADI_ADRV9001_AUXDAC0, &Value);
  else if( Instance->Params->HwVer == 3 )
    status = adi_adrv9001_AuxDac_Code_Get(&Instance->Device, ADI_ADRV9001_AUXDAC3, &Value);
  else
    return Adrv9001Status_InvalidParameter;
...
int32_t Adrv9001_Open( void *devHalCfg )
{
  ...
  if( XSpiPs_SetSlaveSelect(&Adrv9001->Spi, ADRV9001_SPI_CS) != 0 )
    return Adrv9001Status_SpiError;
