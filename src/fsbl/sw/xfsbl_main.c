		...
		case XFSBL_STAGE4:
			{
				...
				/* Default Configuration: OUT1 = 300MHz, OUT2 = 27MHz, OUT3 = 100MHz, OUT4 = 100MHz */
				uint8_t VersaClock5Cfg[] = {0x01,0x0F,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0x01,0x00,0x00,0xB6,0xB4,0x92,0x80,0x2E,0xFD,0xFC,0x00,0x03,0x84,0x06,0xC0,0x00,0x00,0x00,0x9F,0xFD,0xE0,0x80,0x00,0x81,0x02,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x04,0x00,0x00,0x00,0x40,0x00,0x00,0x81,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x04,0x00,0x00,0x03,0x20,0x00,0x00,0x81,0x02,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x04,0x00,0x01,0x00,0xD0,0x00,0x00,0x0C,0x02,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x04,0x00,0x00,0x00,0xD0,0x00,0x63,0x01,0x63,0x01,0x63,0x01,0x63,0x01,0xFF,0x7C};
				...
				do
				{
					...
					if((Config = XIicPs_LookupConfig(XPAR_PSU_I2C_1_DEVICE_ID)) == NULL)
						break;

					if((status = XIicPs_CfgInitialize(&Iic, Config, Config->BaseAddress)) != 0)
						break;

					XIicPs_SetSClk(&Iic, 100e3);

					if((status = XIicPs_MasterSendPolled(&Iic, VersaClock5Cfg, sizeof(VersaClock5Cfg), 0x6A)) != 0)
						break;
				...
				if( status == XST_FAILURE )
					xil_printf("Versa Clock 5 Initialization Failed\r\n");
				else
					xil_printf("Versa Clock 5 Initialization Success\r\n");
