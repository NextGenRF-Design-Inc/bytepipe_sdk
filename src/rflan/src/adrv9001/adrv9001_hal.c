...
//
//  vsnprintf( Buf, 1023, comment, argp );
//
//  adrv9001_evt_data_t EvtData = {
//    .Message = Buf
//  };
//
//  if( Adrv9001Callback != NULL)
//    Adrv9001Callback( Adrv9001EvtType_LogWrite, EvtData, Adrv9001CallbackRef);
//
//  free(Buf);
...
int32_t Adrv9001Hal_ResetbPinSet( void *devHalCfg, uint8_t pinLevel )
{
  ...
  if(pinLevel)
    regVal |= AXI_ADRV9001_CTRL_RSTN_MASK;
  else
    regVal &= ~AXI_ADRV9001_CTRL_RSTN_MASK;
